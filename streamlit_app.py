...
def create_charts(df):
    ...
    with col1:
        ...
        fig = px.histogram(
            df, 
            x='email_count', 
            title='Distribution of Email Counts',
            nbins=20,
    ...
    with col2:
        # Companies with vs without emails
        ...
        fig = px.pie(
            ...
            title='Companies with vs without Emails',
            color_discrete_sequence=['#FF6B35', '#FFA07A']
        )
        st.plotly_chart(fig, use_container_width=True)
...
def main():
    ...
    with tab3:
        ...
        if len(companies_with_emails) > 0:
            ...
            fig = px.bar(
                ...
                y='name',
                orientation='h',
                title='Top 10 Companies by Email Count',
                color='email_count',
    ...
    with tab4:
        ...
        if download_format == "CSV":
            ...
            excel_data = download_data.to_excel(index=False)
            st.download_button(
                label="📥 Download Excel",
                data=excel_data,
                file_name=f"yc_companies_filtered_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
